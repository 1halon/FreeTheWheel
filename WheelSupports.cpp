...
//=============================================================================
//		SetApplierFunctionCopyToCFArray : Applier function for
// CFSetApplyFunction
//-----------------------------------------------------------------------------
...
//=============================================================================
//		ConfigAllDevices : Scan connected devices and apply early
// configs as need
...
bool ConfigAllDevices(const DeviceMode mode) {
  ...
  while (changed == false && tryCount < MAXIMUM_TRIES) {
    tryCount++;
    ...
    CFArrayRef devCFArrayRef =
        CFArrayCreateMutable(kCFAllocatorDefault, 0, &kCFTypeArrayCallBacks);
    CFSetApplyFunction(deviceCFSetRef, SetApplierFunctionCopyToCFArray,
                       (void *)devCFArrayRef);

    // Going through the list, determining their ID and call the appropriate
    // function;
    for (CFIndex i = 0; i < count; i++) {
      IOHIDDeviceRef hidDevice =
          (IOHIDDeviceRef)CFArrayGetValueAtIndex(devCFArrayRef, i);

      UInt32 vendorID = GetPropertyNumber(hidDevice, CFSTR(kIOHIDVendorIDKey));
      UInt32 productID =
          GetPropertyNumber(hidDevice, CFSTR(kIOHIDProductIDKey));
  ...
  printf("Succeeded after %i tries.", tryCount);
...
IOReturn OpenDevice(IOHIDDeviceRef hidDevice) {
  ...
  case kIOReturnNoResources:
    msg = "resource shortage";
    break;
  case kIOReturnIPCError:
    msg = "error during IPC";
    break;
  ...
  case kIOReturnLockedRead:
    msg = "device read locked";
    break;
  case kIOReturnLockedWrite:
    msg = "device write locked";
    break;
  // case kIOReturnExclusiveAccess: msg = "exclusive access and device already
  // open"; break; // Common error on macOS Catalina with DFGT
  case kIOReturnBadMessageID:
    msg = "sent/received messages had different msg_id";
    break;
...
IOReturn SendCommands(IOHIDDeviceRef hidDevice, CCommands *commands) {
  for (int i = 0; i < commands->count; ++i) {
    ...
    IOReturn result =
        IOHIDDeviceSetReport(hidDevice, kIOHIDReportTypeOutput, time(NULL), cmd,
                             kGPCommandMaxLength);
...
//=============================================================================
//		GetPropertyString : Obtain the property string data of the
// device
//-----------------------------------------------------------------------------
...
//=============================================================================
//		GetPropertyString : Obtain the property number data of the
// device
//-----------------------------------------------------------------------------
...
bool ConfigLogitechWheels(IOHIDDeviceRef hidDevice, DeviceID deviceID,
  ...
  if (targetMode == DeviceModeInfoOnly) {
    CFStringRef productID =
        GetPropertyString(hidDevice, CFSTR(kIOHIDProductKey));
    const char *mode =
        native ? sGPLogitechModeNative : sGPLogitechModeRestricted;
  ...
  if (!native) {
    ...
    if (CFStringFind(productID, CFSTR(kGPLogitechG25ProductID), 0).location ==
        0) {
      targetDeviceID = kGPLogitechG25Native;
      printf("Logitech G25 Native mode enabled.\n");
    } else if (CFStringFind(productID, CFSTR(kGPLogitechG27ProductID), 0)
                   .location == 0) {
      targetDeviceID = kGPLogitechG27Native;
      printf("Logitech G27 Native mode enabled.\n");
    } else if (CFStringFind(productID, CFSTR(kGPLogitechG29ProductID), 0)
                   .location == 0) {
      targetDeviceID = kGPLogitechG29Native;
      printf("Logitech G29 Native mode enabled.\n");
    } else if (CFStringFind(productID, CFSTR(kGPLogitechDFGTProductID), 0)
                   .location == 0) {
      targetDeviceID = kGPLogitechDFGTNative;
      printf("Logitech Driving Force GT Native mode enabled.\n");
    } else if (CFStringFind(productID, CFSTR(kGPLogitechDFPProductID), 0)
                   .location == 0) {
      targetDeviceID = kGPLogitechDFPNative;
      printf("Logitech Driving Force Pro Native mode enabled.\n");
    } else if (CFStringFind(productID, CFSTR(kGPLogitechG920ProductID), 0)
                   .location == 0) {
    ...
    if (OpenDevice(hidDevice) == kIOReturnSuccess) {
      ...
      CCommands commands;
      GetCmdLogitechWheelRange(&commands, targetDeviceID,
                               kGPLogitechWheelRangeMax);
    ...
    if (OpenDevice(hidDevice) == kIOReturnSuccess) {
      ...
      if (targetMode == DeviceModeFull) {
        // Activate full 900 degree mode, as being in native mode doesn't
        // guarantee the wheel range
        ...
        // wheel back to 240 degree range
        GetCmdLogitechWheelRange(&commands, deviceID,
                                 kGPLogitechWheelRangeStandard);
        SendCommands(hidDevice, &commands);

        printf("Reset device to default 320 degree wheel movement. "
               "(VendorID/DeviceID %x)\n",
               deviceID);
        changed = true;
      }
...
//=============================================================================
//		GetCmdLogitechWheelNative : Activate native mode for given
// device ID
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void GetCmdLogitechWheelRange(CCommands *c, const DeviceID deviceID,
                              int range) {
  // Fill the command array with null
  memset(c->cmds, 0, kGPCommandsDataSize);
...
//=============================================================================
//		GetCmdLogitechWheelAutoCenter : Deactivate auto-centering for
// given
// device ID
